...
import { supabase, Question, QuestionType, QuestionOption, Materia, MATERIAS, NEMContenido } from '../../lib/supabase'
import { Plus, Trash2, ArrowLeft, Save, Eye, GripVertical, ChevronDown, ChevronUp, Clock, Search } from 'lucide-react'
...
export default function CreateAssignment() {
  ...
  const [contenidosFiltered, setContenidosFiltered] = useState<NEMContenido[]>([])
  ...
  // Cargar contenidos NEM
  useEffect(() => {
    async function loadContenidos() {
      const { data } = await supabase
        .from('nem_contenidos')
        .select('*, campo_formativo:nem_campos_formativos(*)')
        .order('materia')
        .order('orden')
      setContenidos(data ?? [])
    }
  ...
  // Filtrar contenidos por materia seleccionada
  useEffect(() => {
    if (!form.subject) {
      setContenidosFiltered([])
      return
    }
    const filtered = contenidos.filter(c => c.materia === form.subject)
    setContenidosFiltered(filtered)
  }, [form.subject, contenidos])

  // Filtrar por búsqueda
  const contenidosToShow = searchContenido.trim()
    ? contenidosFiltered.filter(c =>
        c.nombre.toLowerCase().includes(searchContenido.toLowerCase()) ||
        c.codigo.toLowerCase().includes(searchContenido.toLowerCase())
      )
    : contenidosFiltered

  useEffect(() => {
    if (!profile?.id) return
    supabase.from('groups').select('id, name, subject').eq('teacher_id', profile!.id).then(({ data }) => setGroups(data ?? []))
    if (isEdit) {
      supabase.from('video_assignments').select('*').eq('id', editId).single().then(({ data }) => {
        if (data) setForm({
          group_id: data.group_id,
          subject: data.subject as Materia,
          title: data.title,
          topic: data.topic,
  ...
  // Auto-select subject from group
  useEffect(() => {
    if (form.group_id && !isEdit) {
      const group = groups.find(g => g.id === form.group_id)
      if (group?.subject) {
        setForm(f => ({ ...f, subject: group.subject as Materia }))
      }
    }
  }, [form.group_id, groups, isEdit])

  const handleSave = async (publish = false) => {
    if (!profile?.id || !form.group_id || !form.title || !form.video_url || !form.subject) {
      alert('Completa: grupo, materia, título y URL del video')
      return
    }

    setSaving(true)

    // Obtener campo formativo automáticamente
    let campoFormativoId = null
    if (form.subject) {
      const contenido = contenidos.find(c => c.id === form.nem_contenido_id)
      campoFormativoId = contenido?.campo_formativo_id ?? null
    }
    ...
    const payload = {
      ...
      nem_campo_formativo_id: campoFormativoId,
      ...
      <div className="space-y-6">
        ...
        <div className="bg-parchment-50 rounded-sm shadow-manuscript border border-parchment-200 p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                value={form.subject}
                onChange={e => setForm(f => ({ ...f, subject: e.target.value as Materia, nem_contenido_id: '' }))}
        ...
        <div className="bg-parchment-50 rounded-sm shadow-manuscript border border-parchment-200 p-6">
          <h2 className="font-display text-lg font-semibold text-ink-800 mb-4 flex items-center gap-2">
            ...
            Contenido NEM
            <span className="text-xs bg-gold-400/20 text-gold-600 px-2 py-0.5 rounded font-mono ml-2">Opcional</span>
          </h2>

          {!form.subject ? (
            <div className="text-center py-8 text-ink-400 font-body text-sm bg-sepia-100 rounded border border-parchment-200">
              Primero selecciona una materia para ver los contenidos NEM disponibles
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Buscar contenido por nombre o código..."
                  className="input-style w-full pl-10 text-sm"
                />
              </div>

              {/* Selector de contenidos */}
              <div className="max-h-60 overflow-y-auto border border-parchment-300 rounded bg-white">
                {contenidosToShow.length === 0 ? (
                  <div className="p-6 text-center text-ink-400 text-sm font-body">
                    {searchContenido.trim() ? 'Sin resultados' : 'Sin contenidos disponibles para esta materia'}
                  </div>
                ) : (
                  <div className="divide-y divide-parchment-200">
                    {contenidosToShow.map(c => (
                      <label
                        key={c.id}
                        className={`flex items-start gap-3 p-3 cursor-pointer transition-colors hover:bg-sepia-100/50 ${form.nem_contenido_id === c.id ? 'bg-gold-400/10' : ''}`}
                      >
                        <input
                          type="radio"
                          name="contenido"
                          checked={form.nem_contenido_id === c.id}
                          onChange={() => setForm(f => ({ ...f, nem_contenido_id: c.id }))}
                          className="mt-1 w-4 h-4 accent-gold-500 flex-shrink-0"
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-mono text-xs bg-crimson-500/10 text-crimson-500 px-2 py-0.5 rounded font-medium">
                              {c.codigo}
                            </span>
                            <span className="text-xs text-ink-400 font-mono">
                              {c.campo_formativo?.codigo}
                            </span>
                          </div>
                          <p className="text-sm font-body text-ink-700 leading-snug">{c.nombre}</p>
                        </div>
                      </label>
                    ))}
                  </div>
              ...
              {form.nem_contenido_id && (
                <button
                  onClick={() => setForm(f => ({ ...f, nem_contenido_id: '' }))}
                  className="text-xs text-ink-500 hover:text-crimson-500 font-body underline"
                >
                  Limpiar selección
                </button>
              )}
        ...
        <div className="bg-parchment-50 rounded-sm shadow-manuscript border border-parchment-200 p-6">
          ...
          <div>
            ...
            <p className="text-xs text-ink-400 mt-1.5 font-body">Compatible con YouTube. Usa el formato: https://www.youtube.com/watch?v=ID del video.</p>
          </div>
        </div>

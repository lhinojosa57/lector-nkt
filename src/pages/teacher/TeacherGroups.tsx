...
import { supabase, Group, MATERIAS, Materia } from '../../lib/supabase'
import { Plus, Users, Copy, Check, Trash2, X } from 'lucide-react'
...
export default function TeacherGroups() {
  ...
  const handleCreate = async () => {
    if (!profile || !form.name.trim() || !form.subject) return
    setSaving(true)
    await supabase.from('groups').insert({ ...form, teacher_id: profile.id })
    setSaving(false)
    setShowModal(false)
    setForm({ name: '', description: '', subject: '', grade: '', school_year: form.school_year })
    loadGroups()
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-crimson-500 text-parchment-50 px-4 py-2.5 rounded-sm font-body font-medium hover:bg-crimson-600 transition-colors shadow-manuscript">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-parchment-50 rounded-sm border border-parchment-200 shadow-manuscript">
          ...
          <button onClick={() => setShowModal(true)} className="bg-crimson-500 text-parchment-50 px-5 py-2.5 rounded-sm font-body font-medium hover:bg-crimson-600 transition-colors">
            Crear grupo
          </button>
        </div>
      ) : (
            ...
            <div key={group.id} className="bg-parchment-50 rounded-sm shadow-manuscript border border-parchment-200 p-5 hover:shadow-raised transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-sm text-ink-500 font-body">{group.subject}</p>
                  {group.grade && <p className="text-xs text-ink-400 font-body">{group.grade} · {group.school_year}</p>}
                </div>
                <button onClick={() => handleDelete(group.id)} className="text-ink-300 hover:text-crimson-500 transition-colors p-1">
        ...
        <div className="fixed inset-0 bg-ink-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-parchment-50 rounded-sm shadow-raised w-full max-w-md animate-slide-up border border-parchment-200">
            <div className="flex items-center justify-between p-6 border-b border-parchment-200">
              <h2 className="font-display text-xl font-semibold text-ink-800">Nuevo grupo</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="ej. Matemáticas 2° A" className="w-full border border-parchment-300 rounded px-3 py-2 font-body text-ink-800 bg-white focus:outline-none focus:border-gold-400 focus:ring-1 focus:ring-gold-400/30" />
              </div>
              <div>
                <label className="text-sm font-body font-medium text-ink-700 block mb-1.5">Materia *</label>
                <select value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value as Materia }))} className="w-full border border-parchment-300 rounded px-3 py-2 font-body text-ink-800 bg-white focus:outline-none focus:border-gold-400">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.grade} onChange={e => setForm(f => ({ ...f, grade: e.target.value }))} placeholder="ej. 2°" className="w-full border border-parchment-300 rounded px-3 py-2 font-body text-ink-800 bg-white focus:outline-none focus:border-gold-400" />
                </div>
                <div>
            ...
            <div className="flex gap-3 p-6 pt-0">
              ...
              <button onClick={handleCreate} disabled={!form.name.trim() || !form.subject || saving} className="flex-1 bg-crimson-500 text-parchment-50 py-2.5 rounded-sm font-body font-medium hover:bg-crimson-600 disabled:opacity-40 transition-colors">

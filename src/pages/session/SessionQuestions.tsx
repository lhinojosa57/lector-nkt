...
export default function SessionQuestions({ textId, onComplete }: Props) {
      ...
      <div className="bg-parchment-50 border-b border-parchment-200">
        <div className="max-w-2xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="font-mono text-sm text-ink-500">{current + 1} / {questions.length}</span>
          </div>
          <div className="h-1.5 bg-parchment-200 rounded-full">
            <div className="h-1.5 bg-teal-500 rounded-full transition-all duration-500"
        ...
        <div className="max-w-2xl w-full" style={{ userSelect: 'none' }}>
          ...
          <div className="space-y-3 mb-6">
            {question.options?.map(opt => {
              ...
              if (submitted) {
                if (opt.id === question.correct_answer) style = 'bg-teal-50 border-teal-500 text-teal-800'
                else if (opt.id === selected && !isCorrect) style = 'bg-red-50 border-red-400 text-red-800'
                else style = 'bg-parchment-50 border-parchment-200 text-ink-400'
              ...
              return (
                <button key={opt.id} onClick={() => !submitted && setSelected(opt.id)}
                  ...
                  <span className="font-mono text-sm font-bold w-6 flex-shrink-0">{opt.id.toUpperCase()})</span>
                  <span className="font-body text-sm leading-snug">{opt.text}</span>
                  {submitted && opt.id === question.correct_answer && <span className="ml-auto text-teal-600 font-bold">✓</span>}
                  {submitted && opt.id === selected && !isCorrect && <span className="ml-auto text-red-500 font-bold">✗</span>}
                </button>
              )
            })}
          </div>
          ...
          {submitted && (
            <div className={`rounded-xl p-4 mb-6 border ${isCorrect ? 'bg-teal-50 border-teal-200' : 'bg-red-50 border-red-200'}`}>
              ...
              <p className="font-body text-sm text-ink-600 leading-relaxed">{question.explanation}</p>
            </div>
          )}

          {!submitted ? (
            <button onClick={handleSubmit} disabled={!selected}
              className="w-full bg-ink-800 text-parchment-100 font-body font-semibold py-4 rounded-xl hover:bg-ink-900 disabled:opacity-40 transition-colors">
              Responder
            </button>
          ) : (
            <button onClick={handleNext}
              className="w-full bg-teal-600 text-white font-body font-semibold py-4 rounded-xl hover:bg-teal-500 transition-colors">
              {current >= questions.length - 1 ? 'Ver cierre →' : 'Siguiente →'}
            </button>
          )}
        </div>
      </div>
